...
class TccIISupabaseUser extends BaseAuthUser {
  TccIISupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> tccIISupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = TccIISupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
